...
export default function TechAssignments() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await apiFetch('/api/tickets?mine=assignee')
  ...
  return (
    <div className="space-y-6">
      ...
      {loading && <div className="flex items-center gap-2 text-gray-600"><Spinner size={18} /> <span>Loading assignments...</span></div>}
      {error && <p className="text-red-600 text-sm">{error}</p>}

      {!loading && tickets.length === 0 && (
        <div className="border rounded-xl p-6 text-center text-gray-600">No assignments yet.</div>
      )}
